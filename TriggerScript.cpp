        ...
        while (running) {
            try {
                for (const auto& entry : config) {
                    ...
                    // Handle process closure first
                    if (!isRunning && running_scripts.find(processName) != running_scripts.end()) {
                        if (entry.config.trigger.type == TriggerConfig::ON_CLOSE) {
                            // Run the close script first
                            HANDLE scriptHandle = runScript(entry.config.scriptPath);
                            if (scriptHandle) {
                                // Wait for the script to finish
                                WaitForSingleObject(scriptHandle, INFINITE);
                                CloseHandle(scriptHandle);
                            }
                        }
                        // Then stop and remove the original script
...
int main() {
    ...
    HANDLE hMutex = CreateMutexW(NULL, TRUE, L"TriggerScript_SingleInstance");
    if (GetLastError() == ERROR_ALREADY_EXISTS) {
        // Find the existing window and bring it to front
        HWND hwnd = FindWindowW(L"ProcessMonitorClass", L"TriggerScript");
        if (hwnd) {
            if (IsIconic(hwnd)) {
                ShowWindow(hwnd, SW_RESTORE);
            }
